...
router.post('/call', (req, res) => {
  ...
  const twiml = telephonyService.handleIncomingCall(cleanNumber);
...
router.post('/keypad', (req, res) => {
  ...
  const cleanNumber = String(fromNumber || '9811223344').replace(/\D/g, '').slice(-10);
  const twiml = telephonyService.handleGather(String(digit), cleanNumber);
...
router.post('/send-sms', (req, res) => {
  ...
  const cleanNumber = String(fromNumber || '9811223344').replace(/\D/g, '').slice(-10);
  const twiml = telephonyService.handleIncomingSMS(cleanNumber, message || 'REGISTER');
