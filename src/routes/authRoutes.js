...
router.post('/send-otp', (req, res) => {
  ...
  dbOps.logTelephony(cleanNumber, 'OUTGOING_OTP', `Your PhoneMail verification code is ${otp}`);
...
router.post('/verify-otp', (req, res) => {
  ...
  if (!user) {
    ...
    user = dbOps.queryOne('SELECT * FROM users WHERE id = ?', [userId]);
  } else if (isMobile && user.has_mobile_app === 0) {
    // Upgrade user to mobile client
    dbOps.execute('UPDATE users SET has_mobile_app = 1 WHERE id = ?', [user.id]);
...
router.post('/portal-register', (req, res) => {
  ...
  dbOps.logTelephony(cleanNumber, 'OUTGOING_NOTIFICATION_SMS', `Welcome to PhoneMail! Your account has been registered via Web Portal.`);
...
router.get('/me', (req, res) => {
  ...
  const aliases = dbOps.queryAll('SELECT * FROM aliases WHERE user_id = ?', [user.id]);
