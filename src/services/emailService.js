...
export const emailService = {
  ...
  async processInboundEmail({ from, to, subject, text, html, attachments = [] }) {
    ...
    let user = dbOps.queryOne('SELECT * FROM users WHERE phone_number = ?', [recipientPhone]);
    if (!user) {
    ...
    if (conversation) {
      ...
      dbOps.execute(`UPDATE conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [conversationId]);
    } else {
    ...
    const savedEmail = dbOps.queryOne('SELECT * FROM emails WHERE id = ?', [emailId]);
  ...
  async sendOutboundEmail({ senderPhone, toRecipients, subject, bodyText, replyToId = null, conversationId = null }) {
    const sender = dbOps.queryOne('SELECT * FROM users WHERE phone_number = ?', [senderPhone]);
    ...
    if (replyToId) {
      const parent = dbOps.queryOne('SELECT * FROM emails WHERE id = ?', [replyToId]);
      if (parent) {
        if (parent.has_replied === 1) {
          throw new Error('This message has already been replied to. Each message can be replied to only once.');
        }
        dbOps.execute('UPDATE emails SET has_replied = 1 WHERE id = ?', [replyToId]);
    ...
    if (!targetConvId) {
      ...
      dbOps.execute('UPDATE conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?', [targetConvId]);
    }
    ...
    const sentEmail = dbOps.queryOne('SELECT * FROM emails WHERE id = ?', [emailId]);
    ...
    for (const rec of normalizedRecipients) {
      ...
      if (parsed.phone && parsed.phone !== senderPhone) {
        const internalUser = dbOps.queryOne('SELECT * FROM users WHERE phone_number = ?', [parsed.phone]);
        if (internalUser) {
